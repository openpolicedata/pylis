...
def get_chapters(year):
    ...
    '''
    r = requests.get(
        f"{_url}/UncodifiedActsGetListOfChapters/{year}", 
...
def get_chapters_sorted_by_subject(year):
    ...
    '''
    r = requests.get(
        f"{_url}/UncodifiedActsGetListOfChaptersBySubject/{year}", 
        verify=_path_to_cert, 
        headers=_headers)
    return json.loads(r.text)
...
def get_chapter_details(year, session, chapter):
    ...
    '''
    r = requests.get(
        f"{_url}/UncodifiedActsGetChapterDetail/{year}/{session}/{chapter}", 
        verify=_path_to_cert, 
        headers=_headers)
    return json.loads(r.text)